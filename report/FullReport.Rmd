...
### 1.2. Comparisons of genes that have at least one TPM in all treatments
...
```{r echo = FALSE, out.width=c('51%', '49%'), fig.height = 5, fig.width = 5, warning=FALSE}
tpms.d.e <- tpms.2 %>% filter(id %in% res.df.elys.sig$id,
                              abs(log2(ElysKD/WT)) > 1) %>% 
         mutate(col = ifelse(chrom == "X", "SpC-specific on X", "SpC-specific on A"))
tpms.d.e$col <- factor(tpms.d.e$col)
p1 <- ggplot(tpms.d.e %>% arrange(col),
       aes(x = log2(WT), y = log2(ElysKD)))+
  geom_point(aes(col = col))+
  ...
  theme_bw()+
  theme(line = element_blank())+
  scale_color_manual(breaks = c("SpC-specific on X", "SpC-specific on A"),
                     values=c("#10DD10", "#DD1010"),
                     name = "")+
