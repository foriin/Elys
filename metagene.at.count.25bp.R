...
bins <- tileGenome(seqinfo(BSgenome.Dmelanogaster.UCSC.dm3),tilewidth = 25,
...
at.npc <- read.table("metagene/metaplot.elys.npc.at.count.50bp.tab", skip = 1)[, -1] %>% t()
at.nuc <- read.table("metagene/metaplot.elys.nuc.at.count.50bp.tab", skip = 1)[, -1] %>% t()
at.oth <- read.table("metagene/metaplot.elys.other.at.count.50bp.tab", skip = 1)[, -1] %>% t()
...
pdf("plots/metagene.AT.percent.50bp.pdf")
p.at
dev.off()
